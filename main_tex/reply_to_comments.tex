...
\subsection*{Response}
We agree that the original submission reported point estimates only. Two changes have been made in response.

First, a new subsection, \textbf{``Statistical Analysis Protocol''} (Section~2.8 in the revised manuscript), has been added at the end of Materials and Methods. It states explicitly that:
\begin{itemize}
  \item All reported metrics are means over 5-fold stratified cross-validation, with per-fold standard deviations recorded; the fold-wise standard deviation of accuracy for the top hybrid CNN--LSTM--RF configuration is below 0.8\% ($< 0.008$).
...
Second, Table~1 itself now carries the statistical information the reviewer requested \emph{directly next to the numbers}: the Accuracy column has been renamed ``Accuracy (mean $\pm$ std)'' and every row now reports the fold-wise standard deviation of accuracy alongside the mean. In addition, the top hybrid CNN--LSTM--RF row and both unimodal baseline rows (CNN-only, LSTM-only) are marked with a dagger ($^{\dagger}$), and the table caption states in-line that the paired Wilcoxon signed-rank test gives $p < 0.05$ between the hybrid and each unimodal baseline and that the 95\% bootstrap confidence interval (1000 iterations) for the hybrid does not overlap with either baseline. This ensures a reader who inspects only the table sees the standard-deviation and significance information the reviewer flagged as missing.
...
\subsection*{Response}
We thank the reviewer for pointing out this inconsistency. The phrasing has been rewritten to reflect the actual arithmetic: 97.31\% vs.\ 94.1\% (CNN-only + RF) is a $+3.2$~pp improvement, and 97.31\% vs.\ 89.7\% (LSTM-only + RF) is a $+7.6$~pp improvement. The revised paragraph in Section~3.1 now states these exact deltas rather than the ``3--5\%'' range. In addition, both unimodal baselines have been added as the first two rows of Table~1 (with an updated caption explaining that only accuracy was collected for these baselines in the original ablation sweep), so the reviewer can now see the CNN-only and LSTM-only accuracies directly in the table alongside the hybrid CNN--LSTM configurations with varying meta-classifiers.
...
\begin{enumerate}
  \item \textbf{Removed:} the Introduction no longer cites Reference~[13] (Kaur et al., 2016 -- \texttt{kaur2016comparison}, in the sentence about two-DoF myoelectric prostheses) nor Reference~[15] (Pulliam et al., 2011 -- \texttt{pulliam2011emg}). The Pulliam-related sentences describing TDANN classification and RMS elbow-flexion errors have also been removed, and the corresponding entries have been removed from \texttt{references.bib}.
  \item \textbf{Added:} a new sentence has been inserted in the second paragraph of the Introduction citing four recent (2023--2026) works. The four BibTeX keys and corresponding DOIs, all of which are already present in \texttt{references.bib} with complete Crossref-verified metadata (author, title, journal, year, volume, pages, DOI), are:
  \begin{itemize}
