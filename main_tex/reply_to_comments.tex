...
Second, Table~1 itself now carries the statistical information the reviewer requested \emph{directly next to the numbers}: the top hybrid CNN--LSTM--RF row and the two unimodal baseline rows (CNN-only, LSTM-only) are marked with a dagger ($^{\dagger}$), and the table caption states in-line that this row has fold-wise standard deviation $< 0.008$, that the paired Wilcoxon signed-rank test gives $p < 0.05$ against both baselines, and that the 95\% bootstrap confidence interval for the hybrid does not overlap with either baseline. The accuracy of the top hybrid row is now reported as $0.9731 \pm 0.008$ inside the cell itself. This ensures a reader who inspects only the table sees the confidence-interval / significance information the reviewer flagged as missing.
