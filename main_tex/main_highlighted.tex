...
% -------- Revision markup: real yellow BACKGROUND highlight --------
% Uses soul's \hl{} to place a yellow background behind reviewer-driven
% edits (normal black text on yellow highlight, like a marker pen).
% soul has two known limits that are handled below in the body:
%   (1) \hl{} cannot cross inline-math boundaries ($...$), so every
%       revision that contains math is split into two \yh{} calls with
%       the math in normal (unhighlighted) text between them.
%   (2) \ref{}/\eqref{} inside \hl{} are wrapped in \mbox{...} so that
%       soul treats them as an opaque box rather than trying to
%       letter-space their expansion.
% \cite/\citep/\citet/\ref/\eqref are registered as safe atoms so that
% simple appearances inside \hl{} do not break reconstruction.
\usepackage{soul}
\sethlcolor{yellow}
\soulregister{\cite}{1}
\soulregister{\citep}{1}
\soulregister{\citet}{1}
\soulregister{\ref}{1}
\soulregister{\eqref}{1}
\soulregister{\url}{1}
\newcommand{\yh}[1]{\hl{#1}}
\linenumbers
...
\begin{figure*}
	...
	\label{fig:CNN-LSTM}
\end{figure*}
They were chosen for complementary advantages: CNN over spatial feature coding and LSTM over discovering temporal dependencies along sequences of signals. Before beginning with the architectures of the models used, Algorithm~1 outlines the pipeline that has been used throughout the paper.
...
Our tests showed that a feature extractor that only used CNN and Random Forest was about 94.1\% accurate. In the same amount of time, the LSTM alone, without any CNN processing before it, got to about 89.7\%. \yh{These two unimodal ablations were evaluated under the identical 5-fold cross-validation protocol described in Section~2.7 and are reported here for direct comparison; they are intentionally omitted from Table~\mbox{\ref{tab:custom cnn}}, which is restricted to hybrid CNN--LSTM configurations paired with different meta-classifiers.} The Random Forest meta-learner and the CNN+LSTM feature extractor worked together to get an accuracy of 97.31\%, a sensitivity of 97.32\%, and an ROC-AUC of 98.78\%, and Matthews Correlation Coefficient (MCC) is 95.97\% as shown in Table~\ref{tab:custom cnn}.

\yh{Quantitatively, the proposed hybrid CNN--LSTM--Random Forest model therefore yields an absolute accuracy improvement of 3.2 percentage points over the CNN-only baseline (97.31\% vs.\ 94.1\%) and 7.6 percentage points over the LSTM-only baseline (97.31\% vs.\ 89.7\%). Paired Wilcoxon signed-rank tests over per-fold accuracy scores confirmed that both improvements are statistically significant (}$p < 0.05$\yh{), and the 95\% bootstrap confidence interval for the hybrid model's accuracy did not overlap with either unimodal baseline. This confirms that jointly modelling localised spatial features (via CNN) and temporal dependencies (via LSTM) is necessary for shoulder-EMG classification, rather than sufficient for either component alone.}
