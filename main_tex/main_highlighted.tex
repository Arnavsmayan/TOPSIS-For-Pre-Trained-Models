...
\begin{table*}[htbp]
	\centering
	\caption{Performance of Unimodal Baselines and Hybrid Models Using Custom CNN as Feature Extractor. Unimodal baseline rows (CNN-only and LSTM-only, both with Random Forest meta-learner) are added per reviewer request; only accuracy is reported for these baselines as the corresponding per-window sensitivity, F1, MCC, and ROC-AUC were not collected in the original ablation sweep.}
	...
	\begin{tabular}{|c|c|c|c|c|c|c|}
		...
		\hline
		CNN only (no LSTM)  & Random Forest & 0.9410 & --- & --- & --- & --- \\
		LSTM only (no CNN)  & Random Forest & 0.8970 & --- & --- & --- & --- \\
		\hline
		\multirow{7}{*}{LSTM} 
