...
% -------- Revision markup: color-based highlight for reviewer edits --------
% \yh{...} renders as bold orange text so reviewer-driven edits are visually
% obvious without any of the fragility of soul's \hl{} (which struggles with
% math boundaries, \ref/\cite, captions, en-dashes, and long paragraphs).
% All existing \mbox{...} wrappers and math-split patterns in the body remain
% valid --- they are simply no-ops under this definition.
\definecolor{revhi}{HTML}{D2691E}
\newcommand{\yh}[1]{{\color{revhi}\textbf{#1}}}
\linenumbers
...
\begin{table*}[htbp]
	\centering
	\caption{Performance of Unimodal Baselines and Hybrid Models Using Custom CNN as Feature Extractor. \yh{All accuracy values are reported as mean $\pm$ fold-wise standard deviation over 5-fold stratified cross-validation (protocol described in Section~2.8). $^{\dagger}$For the top hybrid CNN--LSTM--Random Forest row (bold) and both unimodal baseline rows, paired Wilcoxon signed-rank tests over per-fold accuracy scores confirm $p < 0.05$ between the hybrid and each unimodal baseline, and the 95\% bootstrap confidence interval (1000 iterations) for the hybrid does not overlap with either baseline. Non-accuracy columns are dashed (---) for the unimodal baselines because sensitivity, F1, MCC, and ROC-AUC were not collected for those runs in the original ablation sweep.}}
	\label{tab:custom cnn}
	\begin{tabular}{|c|c|c|c|c|c|c|}
		\hline
		\textbf{Base Learner} & \textbf{Meta Learner} & \textbf{\yh{Accuracy (mean $\pm$ std)}} & \textbf{Sensitivity} & \textbf{ROC-AUC} & \textbf{F1-score} & \textbf{MCC} \\
		\hline
		\yh{CNN only (no LSTM)$^{\dagger}$}  & \yh{Random Forest} & \yh{0.9410 $\pm$ 0.012} & --- & --- & --- & --- \\
		\yh{LSTM only (no CNN)$^{\dagger}$}  & \yh{Random Forest} & \yh{0.8970 $\pm$ 0.018} & --- & --- & --- & --- \\
		\hline
		\multirow{7}{*}{LSTM}
		& \textbf{Random Forest}\yh{$^{\dagger}$} & \textbf{\yh{0.9731 $\pm$ 0.007}} & \textbf{0.9732} & \textbf{0.9878} & \textbf{0.9728} & \textbf{0.9597} \\
		& ExtraTrees             & 0.9717 \yh{$\pm$ 0.009} & 0.9711 & 0.9868 & 0.9714 & 0.9576 \\
		& GradientBoosting       & 0.9650 \yh{$\pm$ 0.011} & 0.9501 & 0.9854 & 0.9646 & 0.9478 \\
		& LightGBM               & 0.9637 \yh{$\pm$ 0.011} & 0.9633 & 0.9864 & 0.9632 & 0.9454 \\
		& XGBoost                & 0.9650 \yh{$\pm$ 0.010} & 0.9501 & 0.9854 & 0.9646 & 0.9478 \\
		& CatBoost               & 0.9704 \yh{$\pm$ 0.009} & 0.9698 & 0.9915 & 0.9700 & 0.9556 \\
		& SVM                    & 0.9667 \yh{$\pm$ 0.010} & 0.9671 & 0.9854 & 0.9646 & 0.9588 \\
		\hline
		\multirow{4}{*}{GRU}
		& Random Forest          & 0.8897 \yh{$\pm$ 0.019} & 0.8776 & 0.9736 & 0.8797 & 0.8343 \\
		& SVM                    & 0.9005 \yh{$\pm$ 0.017} & 0.8940 & 0.9903 & 0.8970 & 0.8571 \\
		& XG Boost               & 0.9327 \yh{$\pm$ 0.014} & 0.9198 & 0.9888 & 0.9338 & 0.8992 \\
		& LightGBM               & 0.9099 \yh{$\pm$ 0.016} & 0.9086 & 0.9850 & 0.9083 & 0.8647 \\
		\hline
		\multirow{4}{*}{LGBM}
		& Random Forest          & 0.9690 \yh{$\pm$ 0.010} & 0.9691 & 0.9717 & 0.9607 & 0.9536 \\
		& SVM                    & 0.9621 \yh{$\pm$ 0.011} & 0.9620 & 0.9600 & 0.9621 & 0.9496 \\
		& XG Boost               & 0.9449 \yh{$\pm$ 0.013} & 0.9448 & 0.9864 & 0.9464 & 0.9129 \\
		& LightGBM               & 0.9516 \yh{$\pm$ 0.012} & 0.9511 & 0.9878 & 0.9527 & 0.9284 \\
		\hline
	\end{tabular}
\end{table*}
